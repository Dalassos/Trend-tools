...
def upgrade_scan():
    if check_dir(rootdir) is True:
        outdir = os.path.join(out_dir(rootdir),"Scan results")
        print(outdir)
        create_dir(outdir)
        try:
            open(os.path.join(outdir, "full_list.csv"),"w").close()     
            open(os.path.join(outdir, "to_upgrade.csv"),"w").close()                
        except:
            ...
            for subdir, dirs, files in os.walk(rootdir):
                for filename in files:
                    if (filename.endswith(".iq") or filename.endswith(".IQ")):
                        ...
                        login(contr_db)
                        dev = scan_controller(contr_db,full,upgrade,outdir)   
                        device = dev[0].split()[0]                        
                        if dev[1]:   #if device is to be upgraded
                            tot_up += 1
                            list_count(dev_up_list,device)
                            #add_replacement(file,ref)
            ...
            full.close()
            upgrade.writelines("Total:,"+str(tot_up)+"\r")   
            total_list(dev_up_list,upgrade)             
            upgrade.close()
            print('Scan complete')
    else:
        print("Not a valid SET Project directory")

def report():
    if check_dir(rootdir) is True:
        outdir = os.path.join(out_dir(rootdir),"Report starter")
        print(outdir)
        create_dir(outdir)
        # try:
            # open(os.path.join(outdir, "full_list.csv"),"w").close()                    
...
def init_gui():
    ...
    e = tk.Button(root, text="Scan for upgrades", wraplength=60, width=10, height=2, command=upgrade_scan)
...
def select():
    global rootdir
    print ("Please choose SET Project file directory")
